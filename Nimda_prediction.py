...
if uploaded_file is not None:
    ...
    def user_input_features():
        ...
        #labels_predictor = st.sidebar.selectbox('Labels(Predictor)',('-1','1'))
...
nimda_raw = pd.read_csv('Nimda_cleaned.csv')
nimda = nimda_raw.drop(columns=['Labels'],axis=1)
df = pd.concat([input_df,nimda],axis=0)
...
load_clf = pickle.load(open('Nimda_clf.pkl', 'rb'))

# Apply model to make predictions
prediction = load_clf.predict(df)
prediction_proba = load_clf.predict_proba(df)
st.subheader('Prediction')
nimda_Labels = np.array(['Normal','Virus'])
st.write(nimda_Labels[prediction])
